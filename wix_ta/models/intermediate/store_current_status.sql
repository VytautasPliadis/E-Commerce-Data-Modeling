-- Extract and format changes data
WITH changes AS (
    SELECT
        store_id,
...
SELECT
    ...
    lpc.removed_at,
    pc.product_name,
    COALESCE((SELECT product_properties FROM product_changes WHERE store_id = lpc.store_id AND product_id = lpc.product_id AND change_type = 'update' AND update_rank = 1), pc.product_properties) AS product_properties,
    CASE
